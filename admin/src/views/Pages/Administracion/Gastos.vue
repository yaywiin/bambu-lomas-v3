  ...
  <AdminLayout>
    <div class="space-y-5 sm:space-y-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h1 class="text-xl sm:text-2xl font-bold text-gray-800 dark:text-white/90">Gastos</h1>
        <button
          @click="openAddModal"
          ...
          <table class="min-w-full">
            <thead>
              <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                ...
                <th class="px-5 py-3 text-left sm:px-6">
                  <p class="font-medium text-gray-500 text-sm dark:text-gray-400">Concepto</p>
                </th>
                <th class="px-5 py-3 text-left sm:px-6">
                  <p class="font-medium text-gray-500 text-sm dark:text-gray-400">Fecha</p>
                </th>
                <th class="px-5 py-3 text-left sm:px-6">
                  <p class="font-medium text-gray-500 text-sm dark:text-gray-400">Monto</p>
                </th>
                <th class="px-5 py-3 text-left sm:px-6">
                  <p class="font-medium text-gray-500 text-sm dark:text-gray-400">Pagado A</p>
                </th>
                <th class="px-5 py-3 text-left sm:px-6">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              >
                ...
                <td class="px-5 py-4 sm:px-6">
                  <p class="text-gray-800 text-sm dark:text-white/90 truncate max-w-[200px]" :title="gasto.concepto">
                    {{ gasto.concepto }}
                  </p>
                </td>
                <td class="px-5 py-4 sm:px-6">
                  <p class="text-gray-500 text-sm dark:text-gray-400">{{ formatFecha(gasto.fecha_pago) }}</p>
                </td>
                <td class="px-5 py-4 sm:px-6">
                  <p class="text-gray-800 text-sm font-medium dark:text-white/90">
                    {{ Number(gasto.monto).toLocaleString('es-GT', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}
                  </p>
                </td>
                <td class="px-5 py-4 sm:px-6">
                  <p class="text-gray-500 text-sm dark:text-gray-400">{{ gasto.pagado_a }}</p>
                </td>
                <td class="px-5 py-4 sm:px-6">
                  <span
                    class="rounded-full px-2.5 py-1 text-xs font-medium"
                    :class="{
                      'bg-success-50 text-success-700 dark:bg-success-500/15 dark:text-success-400': gasto.forma_pago === 'Efectivo',
                      'bg-brand-50 text-brand-700 dark:bg-brand-500/15 dark:text-brand-400': gasto.forma_pago === 'Tarjeta',
                      'bg-warning-50 text-warning-700 dark:bg-warning-500/15 dark:text-warning-400': gasto.forma_pago === 'Transferencia',
                    }"
                  >{{ gasto.forma_pago }}</span>
                </td>
                <td class="px-5 py-4 sm:px-6 flex justify-end gap-2">
                  <button @click="openViewModal(gasto)" class="text-gray-500 hover:text-brand-500 transition-colors" title="Ver">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                    </svg>
                  </button>
                  <button @click="openEditModal(gasto)" class="text-gray-500 hover:text-blue-500 transition-colors" title="Editar">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                    </svg>
                  </button>
                  <button @click="confirmDelete(gasto)" class="text-gray-500 hover:text-error-500 transition-colors" title="Borrar">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="relative w-full max-w-lg sm:my-8 rounded-xl bg-white shadow-lg dark:bg-gray-900 border border-gray-200 dark:border-gray-800 z-50">
          ...
          <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-gray-800">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">
              {{ modalMode === 'add' ? 'Nuevo Gasto' : modalMode === 'edit' ? 'Editar Gasto' : 'Detalle de Gasto' }}
          ...
          <form @submit.prevent="submitForm" class="p-6">
            <div class="space-y-4">
              ...
              <!-- Pagado A -->
              <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Pagado A</label>
                <input
                  type="text"
                  v-model="formData.pagadoA"
                  required
                  :disabled="modalMode === 'view'"
                  placeholder="Persona o empresa a quien se le pagó"
                  class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800 disabled:opacity-75 disabled:bg-gray-50 dark:disabled:bg-gray-800"
                />
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <!-- Fecha -->
                <div>
                  <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Fecha de Pago</label>
                  <div class="relative">
                    <input
                      ref="fechaInputRef"
                      type="date"
              ...
              <!-- Monto -->
              <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Monto</label>
                <input
                  type="number"
            ...
            <div class="mt-6 flex items-center justify-end gap-3 border-t border-gray-200 dark:border-gray-800 pt-5">
              ...
              <button v-if="modalMode !== 'view'" type="submit" :disabled="saving"
                ...
                {{ modalMode === 'add' ? 'Guardar Gasto' : 'Actualizar Gasto' }}
              </button>
            </div>
          </form>
        </div>
    ...
    <Modal v-if="isDeleteModalOpen" :fullScreenBackdrop="true" @close="isDeleteModalOpen = false">
      <template #body>
        <div class="relative w-full max-w-sm sm:my-8 rounded-xl bg-white shadow-lg dark:bg-gray-900 border border-gray-200 dark:border-gray-800 z-50 p-6 text-center">
          ...
          <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">¿Eliminar gasto?</h3>
          <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
            Se eliminará el gasto <strong>#{{ gastoToDelete?.id }}</strong>:
            <strong>{{ gastoToDelete?.concepto }}</strong>.
          </p>
...
const { getGastos, crearGasto, actualizarGasto, eliminarGasto } = useGastos()
...
const populateForm = (g: Gasto) => {
  formData.concepto  = g.concepto
  formData.pagadoA   = g.pagado_a
  formData.monto     = Number(g.monto)
  formData.fechaPago = g.fecha_pago ? g.fecha_pago.toString().substring(0, 10) : ''
  formData.formaPago = g.forma_pago || ''
  formError.value    = ''
}
...
const submitForm = async () => {
  ...
  try {
    ...
    if (modalMode.value === 'add') {
      ...
      gastos.value.unshift(nuevo)
    } else if (modalMode.value === 'edit' && selectedId.value) {
      const updated = await actualizarGasto(selectedId.value, payload)
      const idx = gastos.value.findIndex(g => g.id === selectedId.value)
...
const confirmDelete = (g: Gasto) => {
  gastoToDelete.value = g
...
const doDelete = async () => {
  ...
  saving.value = true
  try {
    await eliminarGasto(gastoToDelete.value.id)
    gastos.value = gastos.value.filter(g => g.id !== gastoToDelete.value!.id)
